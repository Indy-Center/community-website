<script lang="ts">
	...
	const user = $derived.by(() => data.user as User & {
		roles: { role: string }[];
	...
	// Predefined lists for toggle buttons
...
<div class="space-y-6">
		...
		<div class="flex items-start space-x-4">
			...
			<div class="flex-1 space-y-4">
				...
				{#if user.roles.length > 0}
					<div>
						<div class="text-sm text-gray-400 mb-2">System Roles</div>
						<div class="flex flex-wrap gap-1">
							{#each user.roles as roleObj}
								<span class="rounded border px-2 py-1 font-mono text-xs {getRoleBadgeColor(roleObj.role)}">
									{roleObj.role}
								</span>
							{/each}
						</div>
	...
	<div class="rounded-lg border border-slate-700/50 bg-slate-800/50 p-6">
		<div class="mb-4">
			<h2 class="text-lg font-semibold text-white">Certifications</h2>
			<p class="text-sm text-gray-400">Click to toggle certifications for this user</p>
		</div>

		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
			{#each availableCertifications as cert}
				<form method="POST" action="?/toggleCertification" use:enhance={() => {
					return async ({ result }) => {
					...
					<button
						type="submit"
						class="w-full text-left rounded-lg border px-4 py-3 text-sm font-medium transition-all duration-200 cursor-pointer {hasCertification(cert)
							? 'border-green-500 bg-green-600/20 text-green-300 hover:bg-green-600/30'
	...
	<div class="rounded-lg border border-slate-700/50 bg-slate-800/50 p-6">
		<div class="mb-4">
			<h2 class="text-lg font-semibold text-white">Endorsements</h2>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-3">
			{#each availableEndorsements as endorsement}
				<form method="POST" action="?/toggleEndorsement" use:enhance={() => {
					return async ({ result }) => {
						if (result.type === 'success') {
							await invalidateAll();
						}
					...
					<button
						type="submit"
						class="w-full text-left rounded-lg border px-4 py-3 text-sm font-medium transition-all duration-200 cursor-pointer {hasEndorsement(endorsement)
