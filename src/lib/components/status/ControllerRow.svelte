...
<div class="border-b border-slate-700/60 last:border-b-0">
	<button
		class="flex w-full cursor-pointer items-center justify-between gap-3 px-3 py-1.5 text-left transition-colors hover:bg-slate-700/50"
	...
	>
		...
		<div class="flex items-center gap-2 flex-shrink-0">
			{#if primaryPosition}
				<div class="rounded-sm bg-blue-500/20 border border-blue-500/30 px-1.5 py-0.5 font-mono text-xs text-blue-300">
					{controller.vatsimData.callsign || '--'}
				</div>
				<div class="rounded-sm bg-slate-500/20 border border-slate-500/30 px-1.5 py-0.5 font-mono text-xs text-slate-300">
					{formatFrequency(primaryPosition.frequency)}
				</div>
			{:else}
				<div class="rounded-sm bg-slate-500/20 border border-slate-500/30 px-1.5 py-0.5 font-mono text-xs text-slate-400">
					--
				</div>
			{/if}
		</div>
